 ...
 */
static const struct dumtcp_aimd_val {
	unsigned int cwnd;
	unsigned int md;
} dmtcp_aimd_vals[] = {
 {     38,  128, /*  0.50 */ },
...
#define DMTCP_AIMD_MAX	ARRAY_SIZE(dmtcp_aimd_vals)
...
static void dmtcp_init(struct sock *sk)
{
	...
	struct dmtcp *ca = inet_csk_ca(sk);

	ca->ai = 0;

	/* Ensure the MD arithmetic works.  This is somewhat pedantic,
	 * since I don't think we will see a cwnd this large. :) */
	printk(KERN_ALERT "[TCP Dummy] A new dmtcp initiated.\n");
...
static void dmtcp_cong_avoid(struct sock *sk, u32 ack, u32 acked)
{
	...
	if (tp->snd_cwnd <= tp->snd_ssthresh)
		 ...
		 */
		if (tp->snd_cwnd > dmtcp_aimd_vals[ca->ai].cwnd) {
			while (tp->snd_cwnd > dmtcp_aimd_vals[ca->ai].cwnd &&
			       ca->ai < DMTCP_AIMD_MAX - 1)
				ca->ai++;
		} else if (ca->ai && tp->snd_cwnd <= dmtcp_aimd_vals[ca->ai-1].cwnd) {
			while (ca->ai && tp->snd_cwnd <= dmtcp_aimd_vals[ca->ai-1].cwnd)
				ca->ai--;
		}
...
static u32 dmtcp_ssthresh(struct sock *sk)
{
	...
	const struct dmtcp *ca = inet_csk_ca(sk);

	/* Do multiplicative decrease */
	return max(tp->snd_cwnd - ((tp->snd_cwnd * dmtcp_aimd_vals[ca->ai].md) >> 8), 2U);
}
